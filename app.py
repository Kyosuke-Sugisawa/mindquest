...
def quest_all():
    ...
    # --- 検索条件を構築 ---
    sql = "SELECT id, title, description, type FROM quests"
    conditions = []
    params = []
...
@app.route("/admin/quests/edit_steps/<int:quest_id>")
def edit_quest(quest_id):
    ...
    # ここで type_options を code, name のタプルに展開
    type_options = [(code, name) for code, name in type_labels.items()]

    steps_data = json.loads(quest["steps_json"] or "[]")
    return render_template(
        "quest_create.html",
        mode="edit",
        quest_id=quest_id,
        ...
        type_options=type_options  # ← これを渡すことで HTMLに反映される！
    )


@app.route("/admin/quests/update/<int:quest_id>", methods=["POST"])
def update_quest(quest_id):
    print("[DEBUG] form:", request.form)  # ← 追加！
    ...
    cursor = conn.cursor()
    cursor.execute("""
        UPDATE quests SET title=%s, description=%s, type=%s, steps_json=%s
        WHERE id=%s;
    """, (title, description, type_, steps_json, quest_id))
    conn.commit()
    conn.close()
...
def get_type_options():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT code, name FROM types;")
    rows = cursor.fetchall()
    conn.close()
    return rows
...
def create_quest():
    if request.method == "POST":
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO quests (title, description, type, structure, steps_json) VALUES (%s, %s, %s, %s, %s) RETURNING id;",
            (title, description, type_, "multi_step", steps_json)
        )
        row = cursor.fetchone()
        if row is None:
            raise Exception("IDが取得できませんでした")
        quest_id = row["id"]
    ...
    # GET時：テーブルからではなく type_labels を使って表示！
    type_options = [(code, name) for code, name in type_labels.items()]
    return render_template(
        "quest_create.html",
        mode="create",
...
def quest_do(quest_id):
    if request.method == "POST":
        session["quest_form_data"] = request.form.to_dict(flat=False)
    ...
    try:
        ...
        for step in steps_raw:
            ...
            if step_type == "選択式":
                # 選択肢がなければ空リストに
                step["options"] = step.get("options", [])

            elif step_type == "グリッド式":
                # rows, cols を明示的にセット
                step["rows"] = int(step.get("rows", 2))
                step["cols"] = int(step.get("cols", 2))
...
def quest_feedback(quest_id):
    ...
    # 🔸 修正：session からフォームデータを取得
    form_data = session.pop("quest_form_data", {})
    ...
    for i, step in enumerate(steps):
        ...
        if step_type == "グリッド式":
            ...
            for r in range(rows):
                row_data = []
                for c in range(cols):
                    val = form_data.get(f"step_{i}_r{r}c{c}", [""])[0]
                    row_data.append(val)
                grid.append(row_data)
            answer = "\n".join(["｜".join(r) for r in grid])
        else:
            answer = form_data.get(f"step_{i}", [""])[0]
